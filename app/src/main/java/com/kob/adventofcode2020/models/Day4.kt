...
object Day4 {
    fun countValidPassPort(): Int {
        var countValid = 0
        var arrayPassport = ArrayList<String>()
        var temp = array[0]

        for (i in 1 until array.size) {
            if (array[i].isEmpty()) {
                arrayPassport.add(temp)
                temp = ""
            } else {
                temp += " ${array[i]}"
            }
        ...
        for (passport in arrayPassport) {
            var columns = passport.split(" ")

            var count = 0
            for (column in columns) {
                var arrRow = column.split(":")
        ...
        for (passport in arrayPassport) {
            ...
            for (column in columns) {
                ...
                if (arrRow.size == 2) {
                    ...
                    when (key) {
                        "byr" -> {
                            if(checkYear(value,1920,2002)) count++
                        }
                        "iyr" -> {
                            if(checkYear(value,2010,2020)) count++
                        }
                        "eyr" -> {
                            if(checkYear(value,2020,2030)) count++
                        }
                        "hgt" -> {
                            if(checkHeight(value)) count++
                        }
                        "hcl" -> {
                            if(checkHairColor(value)) count++
                        }
                        "ecl" -> {
                            if(checkEyeColor(value)) count++
                        }
                        "pid" -> {
                            if(checkPassportID(value)) count++
                        }
                    }
                }
            }
    ...
    fun checkEyeColor(value: String): Boolean {
        return arrayOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth").filter { it == value }
            .count() > 0
    }
